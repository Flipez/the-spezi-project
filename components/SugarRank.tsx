...
export default function SugarRank({ drinks }: { drinks: Drink[] }) {
  ...
  return (
    <div className="mt-10">
      <h3 className="text-xl font-semibold mb-2">Rank: Sugary vs Sugar-free</h3>

      <p className="text-sm mb-4 max-w-prose">
        Average overall rating of each variant that’s currently in view. Hover bars for exact
        values.
      </p>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 30 }}>
          ...
          <Bar dataKey="avg" name="Average rating" fill="#6366f1" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
